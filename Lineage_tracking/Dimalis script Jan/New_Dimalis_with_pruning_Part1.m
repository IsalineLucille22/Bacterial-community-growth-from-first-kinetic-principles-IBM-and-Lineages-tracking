...
cd('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/ice-clc/D2c/RawData/Anthony_C/Microscopy/Timelapse/231206_07fpr/_1/Pos2')
...
all_pos=readtable('final_merged_table.xlsx');
%all_pos = readtable(strcat('Data/','final_merged_table.xlsx'));
...
s=[]; t=[]; %initiate fresh arrays
s=data(data(:,4)==times(j),:);
t=data(data(:,4)==times(j+1),:);
t2=data(data(:,4)==times(j+2),:);
all_cellIDs=sort(unique(data(:,1)));

% D has the individual distances between cells in s and in t, and s and t2

D{j}=pdist2([s(:,2),s(:,3)],[t(:,2),t(:,3)]);
D{j+1}=pdist2([s(:,2),s(:,3)],[t2(:,2),t2(:,3)]);
...
[E{j}, I{j}]=pdist2([s(:,2),s(:,3)],[t(:,2),t(:,3)],'euclidean','Smallest',1);

% We impose the distance restriction, but use this later.
E{j}(E{j}>threshold)=nan;
[E{j+1}, I{j+1}]=pdist2([s(:,2),s(:,3)],[t2(:,2),t2(:,3)],'euclidean','Smallest',1);
E{j+1}(E{j+1}>threshold)=nan;
...
cells=[]; tells=[]; t2ells=[]; c2ells=[]; %initiate the arrays
