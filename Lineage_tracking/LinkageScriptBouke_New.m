...
% cd('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/ice-clc/D2c/RawData/Anthony_C/Microscopy/Timelapse/220720PP915/_1/Pos1') %To test
...
for i = time_point_init:(len_time_point - 2)
    ...
    m = 1; val_temp = 0; m_1 = 1; val_temp_1 = 0;
    Lineage_1 = Lineage_2;  Lineage_1_1 = Lineage_1;
    Life_time = Life_time_daughter;
    Daughter_temp = zeros(n_1, 2); Daughter_temp_2 = Daughter_temp;
    linear_indices = [];
    ...
    [val, Lineage_ind] = max(h_mat, [], 1); val_1 = val; Lineage_ind_1 = Lineage_ind;
    %%%
    % h_mat_1 = h_mat; %h_mat_1 = ones(n_1, n_2);
    % h_test = h_mat;
...
%%%
    % h_mat_1_1 = ones(n_1, n_2);
    % while m_1 < length(overlap_val_margin_tot) && sum(val_temp_1 == 0) > 0
    %     % overlap_val_margin = overlap_val_margin_tot(m_1);
    % 
    %     %%%
    %     col_inf = find(isinf(sum(h_mat_1_1)));
    %     col_rem = find(sum(h_mat_1_1) < n_1);
    %     while isequal((val_1 == 0), (val_temp_1 == 0)) && m_1 < length(overlap_val_margin_tot)
    %         overlap_val_margin = overlap_val_margin_tot(m_1);
    %         h_mat_1 = H_mat_fun(Dist, n_1, n_2, vect_Cell_length_1 - real_sim*height_cell_1, vect_Cell_length_2 - real_sim*height_cell_2, overlap_val_margin*height_cell_1, overlap_val_margin*height_cell_2, Seg_tot_1, Seg_tot_2, weights, Seg_tot_1_rect, Seg_tot_2_rect, method_number); 
    %         h_mat_1(h_mat_1 < 0) = 0;
    %         h_test = h_mat_1;
    %         h_mat_1(:, col_inf) = h_mat_1_1(:, col_inf);
    %         h_mat_1(:, col_rem) = h_mat_1_1(:, col_rem).*h_test(:, col_rem);
    %         h_test(:, col_rem) = h_mat_1_1(:, col_rem).*h_test(:, col_rem);
    %         [val_1, Lineage_ind_1] = max(h_mat_1, [], 1);
    %         m_1 = m_1 + 1;
    %     end
    % 
    %     Lineage_2_1 = Lineage_1_1(Lineage_ind_1);
    %     nb_zeros = sum(val_1 == 0);
    %     Lineage_2_1(val_1 == 0) = max_Lineage + 1:max_Lineage + nb_zeros;
    %     Lineage_ind_1(val_1 == 0) = 0;
    %     ind_empty = [];
    % 
    %     %%%% To add or remove. Impose unique lineage value for each tree
    %     unique_vals = unique(Lineage_ind_1(Lineage_ind_1 ~= 0));
    %     if ~isempty(unique_vals)
    %         counts = histcounts(Lineage_ind_1, [unique_vals, max(unique_vals) + 1]);
    %         while max(counts) > 2
    %             for j = 1:n_1 
    %                 ind_temp = find(Lineage_ind_1 == j);
    %                 ID_Daughters = cell_ID_2(ind_temp);
    %                 if length(ID_Daughters) > 2
    %                     [~, b] = maxk(h_mat_1(j, ind_temp), 2);
    %                     Ind_Daughters_NK = ind_temp(setdiff(1:length(ID_Daughters), b));
    %                     h_mat_1(j, Ind_Daughters_NK) = 0; h_test(j, Ind_Daughters_NK) = 0;
    %                     ID_Daughters = ID_Daughters(b);
    %                     h_mat_1_1(j, Ind_Daughters_NK) = 0;
    %                 end
    %                 if ~isempty(ID_Daughters)
    %                     Daughter_temp_2(j, :) = ID_Daughters;
    %                 end
    %             end
    %             [val_1, Lineage_ind_1] = max(h_mat_1, [], 1);
    %             Lineage_2_1 = Lineage_1_1(Lineage_ind_1);
    ...
    %             end
    %             %%%
    %             h_mat_1_1(1:n_1~=j, ind_temp) = 0; h_mat_1_1(j, ind_temp) = inf;
    %         end
    %         ind_empty = find(Daughter_temp_2(:,1).*Daughter_temp_2(:,2) == 0);%Remove
    %         [val_1, Lineage_ind_1] = max(h_mat_1, [], 1);
    %         Lineage_2_1 = Lineage_1_1(Lineage_ind_1);
    %         nb_zeros = sum(val_1 == 0);
    %         Lineage_2_1(val_1 == 0) = max_Lineage + 1:max_Lineage + nb_zeros;
    ...
    %     end
    %     %%%%%
    %     ind_daughter_1 = 1:n_2;
    %     ind_daughter_1(val_1 == 0) = [];
    %     m_1 = m_1 + 1;
    %     val_temp_1 = val_1;
    % end
    % 
    % if ~isequal(Lineage_ind, Lineage_ind_1)
    %     c = 10; 
    % end
    % inf_h_mat = find(isinf(h_mat)); inf_h_test = find(isinf(h_mat_1));
    % if sum(inf_h_mat - inf_h_test) ~= 0 %~isequal(inf_h_mat, inf_h_test)
    %     c = 10;
    % end
    % Daughter_temp = Daughter_temp_2; Lineage_ind = Lineage_ind_1; %h_mat = h_mat_1;
    % h_test(inf_h_test) = inf; h_mat = h_test;
    % if ~isequal(h_test, h_mat)
    %     c = 10; 
    % end
...
lin_to_show = [5]; %1:nb_Lin %Insert the lineages to show. Warning: if there is a lot a lineage, don't show all of them, this will produce a lot of figures and Matlab can crash
