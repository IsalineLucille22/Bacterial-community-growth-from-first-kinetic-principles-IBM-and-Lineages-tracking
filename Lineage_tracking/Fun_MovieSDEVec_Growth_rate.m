function [Tot_res] = Fun_MovieSDEVec_Growth_rate(Data, real_sim, save_data, name_movie_file, initial_time_point)
...
colormap_used = jet(256); % Choose a colormap (e.g., 'jet', 'parula', etc.)
colors_set_lineage_col = colormap_used(max(ceil(mu_max_normalized*length(colormap_used)), 1), :); % Map to colormap
