function [Tot_res] = Fun_MovieSDEVec_Lineage(res_number_ind, Data, save_data, name_movie_file, Delta_Time)
load(Data)
...
colors_set_lineage = distinguishable_colors(50);
scale_factor = 1;
...
x_axis = [(dim_Img(1, 1) - dx/2) (dim_Img(1, 2) + dx/2)];
y_axis = [(dim_Img(2, 1) - dy/2) (dim_Img(2, 2) + dy/2)];
fig = {};
for i = 1:Nb_species
...
while k < n_step
    ...
    rho_temp = rho_3D_tot{res_number_ind, (k-1)*p_Diff + 1};
    rho_temp = rho_temp(:,:,1);
    rho_temp(bbRegion) = nan;
    im = image(x_axis, y_axis, rho_temp, 'CDataMapping','scaled'); %image(x_axis, y_axis, rho_temp, 'CDataMapping','scaled');
    im.AlphaData = 0.5;
    for i = 1:Nb_species
        if index_species(i) ~= 9
            index_lineage_temp = num_col{i,(k-1)*p_Diff + 1};
            ...
            if ~isempty(vect_Cell_Length_temp)
                  rectangle2([P_temp_S(1,:)' P_temp_S(2,:)'  (vect_Cell_Length_temp + height_cell(index_species(i)))' height_cell(index_species(i))*ones(length(vect_angle_temp),1)],'Curvature',...
                    [min((length_cell(index_species(i)) + height_cell(index_species(i)))./(vect_Cell_Length_temp + height_cell(index_species(i)))'*0.5882.*ones(length(vect_angle_temp),1), 1) 1*ones(length(vect_angle_temp),1)],...
