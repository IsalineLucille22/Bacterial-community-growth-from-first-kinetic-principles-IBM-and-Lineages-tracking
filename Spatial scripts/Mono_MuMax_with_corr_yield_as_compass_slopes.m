...
% This variant uses simulations with mu's and yields from the monocultures (files named 'SynCom7_SE')
% The yields are then corrected by the observed pair-wise slopes (compass values in Fig. 4e)

clear
close all


%%%%%Initialization cell positions based on real data%%%%%
...
%Slope in the 2nd column indicates the effect of the 2nd species on the growth rate of the first species (so it is the max umax of the 1st species that is modified). For now that means I have to invert the numbers in the csv to the other partner, because I explained wrong to Isaline.

mu_Mody = readtable(strcat('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/sinergia/D2c/UserData/Isaline/Model/', 'yield_correction_table.csv'));

%change the measured slope values in pairs for the effect on the final biomass yield

% mu_Mody.mean_slope=mu_Mody.mean_slope./(1+mu_Mody.mean_slope);

%Now we still have to take into account how cells can actually gain additional yield from a partner? Maybe from the mean of the partner sum yields?

Data_path = strcat(path_init, 'Data/');
cd(path_Data_position)

init_name = 'IG_SynCom7_SE_mono_yield_with_competition_index_corrections';
...
for zz = 1:nb_pos
    ...
    Name_file = strcat(init_name, '_Pos_', num2str(zz));
    ...
    lag_time = Mean_Cell(lag_time); %We will pass the lag time as Data for the SDEsSpat3D_2.m function
    ...
    for i = 1:(nb_species)
        ...
        mu_max(i) = mu_max_data.mean_mu(ind_mu_max);
    ...
    %For each cell present in the experiment, check neighbour cells from
    ...
    %Only iterate on the first column because of the symmetry of the pair
    ...
    pos_res_prod = 2;
    Prod_Mat = Init_Prod_Mat(kappa_3, nb_species, nb_res, pos_res_prod);  
    %Production rate matrices. 
    %Each row of each matrix corresponds to a resource and the rates
    %indicate which byproduct can be produced from the resource. Sum on the
    %row then indicates kappa_3 for a resource.
    ...
    [~, ~, Pos_S, vect_Cell_Length_tot, Mass_Cell_Evol, vect_angle_tot, ~, num_col, Generation_tree, Mass_Res_Waste_Evol, rho_vect, ~, lag_time, bbRegion, ~, rho_3D_tot, Vect_Time_saved] = ...
        SDEsSpat3D_2(lag_time, vect_species, T_fin, t_D, Diff_Coeff, t_Diff, dx, dy, Time_forces, Time_saved, length_cell, height_cell, Mass_S_0, Mass_R_W_0, Mass_Vol, Vol_Box, lz,...
    ...
    for j = 1:nb_species
        ...
        scatter(ax2, final_area_px_sim(j), final_area_px_obs(j), 20, color_set(j,:), 'o', 'filled');
        scatter(ax3, final_nb_cells_sim(j), final_nb_cells_obs(j), 20, color_set(j,:), 'o', 'filled');
    end
...
writetable(T, strcat(Data_path, init_name, '_Comp_Sim_Obs_Stat_biomass_Yields_IG.xlsx'));