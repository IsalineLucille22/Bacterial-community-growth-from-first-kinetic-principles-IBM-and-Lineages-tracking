...
% which are then corrected by the observed pair-wise influence on the paired growth rates
...
%mu_Mody = readtable(strcat('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/sinergia/D2c/UserData/Isaline/Model/', 'mean-observed-paired-slopes.csv')); %Slope in the 2nd column indicates the effect of the 2nd species on the final area of the 1st species

%mu_Mody = readtable(strcat('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/sinergia/D2c/UserData/Isaline/Model/', 'mean-observed-paired-mu-corrections.csv'));

%Slope in the 2nd column indicates the effect of the 2nd sepcies on the growth rate of the first species (so it is the max umax of the 1st species that is modified). For now that means I have to invert the numbers in the csv to the other partner, because I explained wrong to Isaline.

mu_Mody = readtable(strcat('/Volumes/RECHERCHE/FAC/FBM/DMF/jvanderm/sinergia/D2c/UserData/Isaline/Model/', 'mean-observed-paired-mu-corrections-inverted.csv'));

Data_path = strcat(path_init, 'Data/');
cd(path_Data_position)

init_name = 'SynCom7_SE_mono_growth_rate_with_umax_pair_corrections';
...
for zz = 1:nb_pos
    ...
    [fact_mod_mu, fact_mod_mu_2, nb_appears] = deal(zeros(nb_cells, 1));
    ...
    fact_mod_mu_2(isnan(fact_mod_mu)) = 1;
...
writetable(T, strcat(Data_path, init_name, '_Comp_Sim_Obs_Stat_biomass.xlsx'));