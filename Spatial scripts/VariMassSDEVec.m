function [Mass_Cell, Mass_Res, vect_Cell_length_temp, P_S_temp, Ind_Cell_Rem, Generation_tree_temp, num_col_tot, mu_evol_mean_temp, Scale_Division, mu_max_cell, d_temp_S1R, Index_Res, duplicate_indices, C, ic, Mat_rate, kappa_1, kappa_2, kappa_3, shrink_rate_cell, Threshold_Species_Res] =...
    VariMassSDEVec(Index_Res, Mass_Cell, vect_Cell_length_temp, Mass_Res, vect_rate, Vol_mass_cell, height_cell, Threshold_Species_Res, t_D, P_S_temp, nb_species, index_species, Threshold_division, Scale_Division, Vol_Box, Generation_tree_temp, num_col_tot, lag_time, t, mu_max_cell, d_temp_S1R, Pos_Resource, duplicate_indices, C, ic, Mat_rate, kappa_1, kappa_2, kappa_3, shrink_rate_cell, dx)
...
nb_res_prod = length(idx_Res_prod);
...
%Make the average concentration on the two density resource boxes.
...
%Threshold on time
Threshold = max(t - Threshold_Species_Res, 0)./(t - Threshold_Species_Res); %Matrix n_resxn_cells
Threshold(isnan(Threshold)) = 0;
% Oxygen_Mat(1:(end - 1),:) = repmat((Surr_Mass_R(end,:)./(Surr_Mass_R(end,:) + (abs(kappa_2(end,:)) + abs(kappa_3(end,:)))./kappa_1(end,:))).^1, nb_res_cons - 1, 1); %If there is no effect of the oxygen (anaerobic cells), comment it
% Oxygen_Mat(1:(end-1),:) = repmat(max((1 - (9e-03 - Surr_Mass_R(end,:))./9e-03),0), nb_res_cons - 1, 1); %
...
%Addition here cells on divison by 1/2 or computed weights for each C_3, C_4
...
for i = 1:nb_res_prod
    kappa_prod_temp = reshape(Mat_rate(:, idx_Res_prod(i) + 2,:), nb_res_cons, nb_cells); %+2 because columns 1-2 are kappa_1 and kappa_2
    New_Mass_R = t_D*Threshold.*(Mass_Cell_temp.*kappa_prod_temp.*denom_temp);
    Mass_Res(idx_Res_prod(i), C) = Mass_Res(idx_Res_prod(i), C) + accumarray(ic, sum(New_Mass_R))';
end

%Sources of resources
% Mass_Res(1,678) = 3*9.4732e-14;%Mass_Res(1,6760) + t_D*5*(max(t,10))*7.4604e-11;%t_D*5/(max(t,5))*50*7.4604e-13;381
...
New_length_cell = Mass_Cell./(pi*Vol_mass_cell(vect_species).*(height_cell(vect_species)/2).^2)' - 4/6*height_cell(vect_species)';%Mass_Cell./(Vol_mass_cell(vect_species).*height_cell(vect_species))'-pi*height_cell(vect_species)'/4;
...
if ~isempty(ind_division)
    ...
    New_length_cell(ind_division) = Mass_Cell(ind_division)./(pi*Vol_mass_cell(vect_species(ind_division)).*(height_cell(vect_species(ind_division))/2).^2)' - 4/6*height_cell(vect_species(ind_division))';%(Mass_Cell(ind_division)./(Vol_mass_cell(vect_species(ind_division)).*height_cell(vect_species(ind_division)))'-pi*height_cell(vect_species(ind_division))'/4);
    New_length_cell = [New_length_cell (Mass_Cell_new./(pi*Vol_mass_cell(vect_species(ind_division)).*(height_cell(vect_species(ind_division))/2).^2)' - 4/6*height_cell(vect_species(ind_division))')];%[New_length_cell (Mass_Cell_new./(Vol_mass_cell(vect_species(ind_division)).*height_cell(vect_species(ind_division)))'-pi*height_cell(vect_species(ind_division))'/4)];
    ...
    max_gen = max(Generation_tree_temp);
    Generation_tree_temp(ind_division) = max_gen  + (1:2:(2*nb_division)); %Label the dividing cell as #cells + 1 and #cells + 2
    Generation_tree_temp = [Generation_tree_temp (max_gen + (2:2:(2*nb_division + 1)))];
    num_col_tot = [num_col_tot num_col_tot(ind_division)];
    Scale_Division(ind_division) = Scale_Division(ind_division);%max(Scale_Division(ind_division)*1/(1 + exp(-1.2)), 0.57); %Decreasing factor (1-0.5) reprensenting the decrease on the divison threshold
    Scale_Division = [Scale_Division Scale_Division(ind_division)];
    mu_max_cell = [mu_max_cell max(mu_max_cell(:, ind_division) + [normrnd(0, 0.05, 1, length(ind_division)); zeros(5, length(ind_division))], 0)]; %Remove last part or modify it if variation in several nutrients 
...
%     vect_species = vect_Cell_length_temp(2,:);
    Mat_rate_old = Mat_rate;
    Mat_rate = zeros(nb_res_cons, nb_res_cons + 2, nb_cells + nb_division);
    Mat_rate(:,:, 1:nb_cells) = Mat_rate_old;
    Mat_rate(:,:, (nb_cells + 1:nb_cells + nb_division)) = Mat_rate_old(:,:, ind_division);
%     for i = 1:nb_species
...
%     end
    ...
    kappa_3 = reshape(sum(Mat_rate(:,3:end,:),2), nb_res_cons, nb_cells + nb_division); %Sum the consumed resources kappa_3
