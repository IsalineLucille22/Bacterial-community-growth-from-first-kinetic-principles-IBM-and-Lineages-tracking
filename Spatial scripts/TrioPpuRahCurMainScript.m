...
init_name = 'Trio_Script_PpuCurRahMuc_Mono_wo_fact_';
addpath(path_init)
...
% yield(3,:) = 3*yield(3,:);
nb_res = 6;%Change according to the consumption, production matrices size below
...
nb_pos = 1;
[tot_final_area_px_sim, tot_final_nb_cells_sim, tot_final_area_px_obs, tot_final_nb_cells_obs] = deal(zeros(4, nb_pos));
for zz = 1:nb_pos
    ...
    Data_nb_cells = readtable(strcat(path_init,'Data/Pos6PpuCurRah/', 'manual_reclass.csv'));%readtable(strcat(path_Data_position, Position, 'manual_reclass.csv'));
       
    Data_kept_Pos = Data_nb_cells; 
